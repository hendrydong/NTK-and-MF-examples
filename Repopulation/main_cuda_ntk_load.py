...
class NTK(nn.Module):
    ...
    def __init__(self, h_dim):
        ...
        self.net0.weight = nn.Parameter(vae.predict((self.net0.weight)))
        self.net_nt = nn.Linear(784, 10*h_dim, bias=True).cuda() # u d theta
        ...
        self.out = lambda x:x#nn.Softmax(dim = 1)
...
for s in range(save_iter):
    ...
    np.save('./results/loss_ntk_load_%d_%d'%(h_dim,s),loss_full)
    np.save('./results/acc_ntk_load_%d_%d'%(h_dim,s),acc_full)
