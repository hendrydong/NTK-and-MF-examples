...
for s in range(save_iter):
    ...
    c = np.random.choice(np.arange(10000),size = h_dim)
    w = weight[c]
    ...
    if save:
        ...
        np.save('./results/mnist_s/acc_us_%d'%s,acc_full)
        np.save('./results/mnist_s/loss_us_%d'%s,loss_full)
        np.save('./results/mnist_s/u_us_%d'%s,u_full)
